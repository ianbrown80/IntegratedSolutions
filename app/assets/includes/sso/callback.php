...
// Get the Character details from SSO
  ...
  $sql="SELECT corporationname, corporationticker, user.corporationid, alliancename, allianceticker,
    corporation.allianceid, characterid, characterownerhash, user.id FROM user
    ...
    WHERE user.characterid=:characterid AND characterownerhash=:characterhash";
  ...
  //Alliance
    if ($allianceID!=0) {
      ...
      if (!isset($allianceticker)) {
        ...
        $stmt = $pdo->prepare($alliance_insert_sql);
        $stmt->execute(
          array(
    ...
    $stmt = $pdo->prepare($corporationsql);
    ...
    if (!isset($corporationticker)) {
      ...
      $stmt = $pdo->prepare($corporation_insert_sql);
      $stmt->execute(
        array(
    ...
    $stmt = $pdo->prepare($user_creation_sql);
    $stmt->execute(
      array(
